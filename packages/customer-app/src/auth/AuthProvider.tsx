...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [keycloak] = useState(
    () =>
      new Keycloak({
        ...
        realm: import.meta.env.VITE_KEYCLOAK_REALM || 'kosmetika',
        clientId: import.meta.env.VITE_KEYCLOAK_CLIENT_ID || 'kosmetika-customer',
      }),
  );
  ...
  useEffect(() => {
    keycloak
      ...
      .catch(() => {
        // Keycloak not available - allow anonymous usage
        setIsInitialized(true);
      });
  }, [keycloak]);
